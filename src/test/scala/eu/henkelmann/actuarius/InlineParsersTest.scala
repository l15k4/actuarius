 ...
 */
object InlineParsersTest extends TestSuite with InlineParsers{
    ...
    val tests = TestSuite {
        ...
        def runExceptionParsingTests(p: Parser[String], l: List[String]) {
            for (s <- l) intercept[IllegalArgumentException] {
                apply(p, s)
            }
        }
        ...
        "it should create fast links in" - {
            ...
            intercept[IllegalArgumentException] {
                apply(p, "<this is not a fast link<span>")
            }
        ...
        "it should resolve references in" - {
            val p = ref(ctx)
            assert(apply(p, "[text][id]") ==(ld1, "text"))
            assert(apply(p, "[text] [id]") ==(ld1, "text"))
            assert(apply(p, "[id][]") ==(ld1, "id"))
            assert(apply(p, "[id] []") ==(ld1, "id"))
            assert(apply(p, "[id]") ==(ld1, "id"))
            assert(apply(p, "[Id]") ==(ld1, "Id"))
        }
        ...
        "it should resolve reference links in" - {
            val p = inline(map)
            assert(apply(p, "[text][id]") == """<a href="http://www.example.com" title="Title">text</a>""")
            assert(apply(p, "[text] [id]") == """<a href="http://www.example.com" title="Title">text</a>""")
            assert(apply(p, "[id][]") == """<a href="http://www.example.com" title="Title">id</a>""")
            assert(apply(p, "[id] []") == """<a href="http://www.example.com" title="Title">id</a>""")
            assert(apply(p, "[id]") == """<a href="http://www.example.com" title="Title">id</a>""")
            assert(apply(p, "[Id]") == """<a href="http://www.example.com" title="Title">Id</a>""")

            assert(apply(p, "[id] [Id 2]") == """<a href="http://other.example.com" title="Title 2">id</a>""")
            assert(apply(p, "[id 3]") == """<a href="http://none.example.com">id 3</a>""")
            assert(apply(p, "[foo \"bar\"][id 3]") == """<a href="http://none.example.com">foo &quot;bar&quot;</a>""")
        }

        "it should resolve reference images in" - {
            val p = inline(map)
            assert(apply(p, "![text][id]") == """<img src="http://www.example.com" alt="text" title="Title" />""")
            assert(apply(p, "![text] [id]") == """<img src="http://www.example.com" alt="text" title="Title" />""")
            assert(apply(p, "![id][]") == """<img src="http://www.example.com" alt="id" title="Title" />""")
            assert(apply(p, "![id] []") == """<img src="http://www.example.com" alt="id" title="Title" />""")
            assert(apply(p, "![id]") == """<img src="http://www.example.com" alt="id" title="Title" />""")
            assert(apply(p, "![Id]") == """<img src="http://www.example.com" alt="Id" title="Title" />""")

            assert(apply(p, "![id] [Id 2]") == """<img src="http://other.example.com" alt="id" title="Title 2" />""")
            assert(apply(p, "![id 3]") == """<img src="http://none.example.com" alt="id 3" />""")
            assert(apply(p, "![foo \"bar\"][id 3]") == """<img src="http://none.example.com" alt="foo &quot;bar&quot;" />""")
        }
